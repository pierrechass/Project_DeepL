...
We will in a second time compare the two numbers to see which one is the biggest. COmpare to Test6, in this case we compute the target
manualy once we found the classes of the images . In test6 we will add layers to our Neural network to find the target. 
"""
...
def train_model(model, train_input, train_target, mini_batch_size, nb_epochs = 25):
    ...
    criterion =nn.CrossEntropyLoss()
    eta = 1e-2
    momentum = 0.0 # best score without momentum
