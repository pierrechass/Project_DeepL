...
We will in a second time compare the two numbers to see which one is the biggest. COMPARE TO Test3 but we added some batch normalization
"""
...
[train_input, train_target, train_classes, test_input, test_target, test_classes]=  prologue.generate_pair_sets(N)
    ...
    # optimizer: Adam 
    # nb_hidden : 200
    # score  :  4.45    testing set classes    
    #           3.18    testing set target
    #           0.1949    training set classe
    #           0.1  training set target
    #
    ...
    # optimizer: SGD
    # nb_hidden : 200
    # score  :  6.74   testing set classes    
    #           4.59    testing set target
    #           4.845    training set classe
    #           3.37    training set target
    #
    # criterion : nn.crossEntropyLoss()
...
def compute_nb_errors_target(model, data_input, data_target):
    ...
    for k in range(A):
        if predicted_classes[2*k]>predicted_classes[k*2+1]: # if the same number, choose the second image-> sol=torch.tensor(1)
            sol=torch.tensor(0)
        else:
            sol=torch.tensor(1)
